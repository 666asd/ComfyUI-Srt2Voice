...
def build_semantic_model(path_='./models/tts/maskgct/ckpt/wav2vec2bert_stats.pt', w2v_model_path=None):
    if w2v_model_path is None:
        # Fallback to default HuggingFace repo if no local path provided
        w2v_model_path = "facebook/w2v-bert-2.0"
    semantic_model = Wav2Vec2BertModel.from_pretrained(w2v_model_path)
    semantic_model.eval()
