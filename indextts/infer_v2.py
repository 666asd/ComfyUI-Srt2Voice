...
class IndexTTS2:
    def __init__(
        ...
        # Load SeamlessM4TFeatureExtractor from local model_dir
        w2v_bert_path = os.path.join(self.model_dir, "models--facebook--w2v-bert-2.0")
        if os.path.exists(w2v_bert_path) and os.path.exists(os.path.join(w2v_bert_path, "preprocessor_config.json")):
            self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(w2v_bert_path, local_files_only=True)
        else:
            # Download to specified directory if not exists
            print(f">> Downloading w2v-bert-2.0 to {w2v_bert_path}...")
            os.makedirs(w2v_bert_path, exist_ok=True)
            from huggingface_hub import snapshot_download
            snapshot_download(
                repo_id="facebook/w2v-bert-2.0",
                local_dir=w2v_bert_path,
                local_dir_use_symlinks=False
            )
            self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(w2v_bert_path, local_files_only=True)
        self.semantic_model, self.semantic_mean, self.semantic_std = build_semantic_model(
            os.path.join(self.model_dir, self.cfg.w2v_stat),
            w2v_model_path=w2v_bert_path
        )
        ...
        # Load semantic_codec from local model_dir instead of downloading
        semantic_codec_dir = os.path.join(self.model_dir, "models--amphion--MaskGCT")
        semantic_code_ckpt = os.path.join(semantic_codec_dir, "semantic_codec", "model.safetensors")
        if not os.path.exists(semantic_code_ckpt):
            # Download to specified directory if not exists
            print(f">> Downloading semantic_codec to {semantic_codec_dir}...")
            os.makedirs(semantic_codec_dir, exist_ok=True)
            from huggingface_hub import hf_hub_download
            semantic_code_ckpt = hf_hub_download(
                "amphion/MaskGCT",
                filename="semantic_codec/model.safetensors",
                local_dir=semantic_codec_dir,
                local_dir_use_symlinks=False
            )
        ...
        # Load BigVGAN from local model_dir
        bigvgan_name = self.cfg.vocoder.name
        # Convert name like "nvidia/bigvgan_v2_22khz_80band_256x" to "models--nvidia--bigvgan_v2_22khz_80band_256x"
        bigvgan_dir_name = "models--" + bigvgan_name.replace("/", "--")
        bigvgan_path = os.path.join(self.model_dir, bigvgan_dir_name)
        
        # Check if required files exist (BigVGAN only needs config.json and bigvgan_generator.pt)
        config_file = os.path.join(bigvgan_path, "config.json")
        model_file = os.path.join(bigvgan_path, "bigvgan_generator.pt")
        ...
        if os.path.exists(config_file) and os.path.exists(model_file):
            ...
            print(f">> Downloading BigVGAN to {bigvgan_path}...")
